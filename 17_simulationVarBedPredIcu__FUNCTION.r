...
for (s in 1:nsimulation){
  dataM = data[data$lengthofstay<quantile(data$lengthofstay, 0.975), ]
  dataM = data[,-1]
  prune.control <- rpart.control(xmin = 35, cp = -1, minsplit=15, minbucket = 25)
  model = rpart(dataM$lengthofstay ~.,  data = dataM, control = prune.control)
    ...
    if(i == admissionRate$hour[a]){
      ...
      if(i>=28*24 && i%%24==0){
        ...
        degAtt = sum(rpois(sum(!is.na(hourClockBeds)), lambda = (mean(CLosU, na.rm = T)*sum(CRatioU, na.rm = T)+mean(CLosE, na.rm = T)*(length(CAdmRate)-sum(CRatioU, na.rm = T)))/length(CAdmRate)))
          ...
          degAtt = sum(predict(model, data[data$admission%in% idBeds, ]))
